...
def validate_schedule(
    ...
    *,
    ...
    linked_sets = [
        {str(code).upper() for code in group}
        for group in (linked_groups or [])
    ]

    def linked_together(indices: Iterable[int]) -> bool:
        index_set = set(indices)
        linked_units = {
            str(df.loc[index].get("linked_unit", "") or "")
            for index in index_set
        }
        if len(linked_units) == 1 and "" not in linked_units:
            return True
        section_groups = {
            str(df.loc[index].get("co_schedule_group", "") or "")
            for index in index_set
        }
        if len(section_groups) == 1 and "" not in section_groups:
            return True
        base_codes = {
            str(df.loc[index].get("base_code", df.loc[index, "code"])).upper()
            for index in index_set
        }
        sections = {
            int(df.loc[index].get("section", 1))
            for index in index_set
        }
        split_indices = {
            int(df.loc[index].get("split_index", 0))
            for index in index_set
        }
        return (
            len(base_codes) > 1
            ...
            and any(base_codes <= group for group in linked_sets)
        )
    ...
    for (d, s), idxs in occ.items():
        ...
        for inst, conflicting in by_instructor.items():
            if len(conflicting) > 1 and not linked_together(conflicting):
    ...
    if check_semester and not df.empty:
        report.soft_overlap_count = _validate_semester(
            df,
            occ,
            report,
            linked_sets=linked_sets,
        )
...
def _validate_semester(
    ...
    report: ValidationReport,
    *,
    linked_sets: list[set[str]] | None = None,
) -> int:
    ...
    def linked_together(indices: set[int]) -> bool:
        linked_units = {
            str(df.loc[index].get("linked_unit", "") or "")
    ...
    for (d, s), idxs in occ.items():
        ...
        for (sem, sec), mand in mandatory_groups.items():
            ...
            if len(here) > 1 and not linked_together(here):
            ...
            if here:
                ...
                if elec_here and not linked_together(here | elec_here):
                    codes = sorted(
...
def validate_global_instructor(final_df: pd.DataFrame) -> list[Violation]:
    ...
    # (day, slot) -> {instructor: {independent teaching-unit: codes}}.
    # Sections intentionally synchronized by co_schedule_group are one teaching
    # unit even when the same instructor appears on several section rows.
    occ: dict[tuple[int, int], dict[str, dict[str, set[str]]]] = {}
    for _, row in final_df.iterrows():
        day = int(row["d_idx"])
        slot_indices = row.get("slot_indices")
        if isinstance(slot_indices, (list, tuple)) and len(slot_indices):
            slots = [int(value) for value in slot_indices]
        else:
            slots = [int(row["s_idx"])]
        code = str(row.get("DersKodu", row.get("code", "?")))
        instructor = str(row.get("instructor", "-")).strip()
        ...
        teaching_unit = f"group:{section_group}:{split_index}" if section_group else f"course:{code}"
        for slot in slots:
            units = occ.setdefault((day, slot), {}).setdefault(instructor, {})
            units.setdefault(teaching_unit, set()).add(code)

    for (day, slot), by_instr in occ.items():
        for instructor, units in by_instr.items():
            if len(units) > 1:
                codes = sorted({code for grouped_codes in units.values() for code in grouped_codes})
                violations.append(Violation(
                    "H4", "error",
                    f"instructor '{instructor}' teaches {len(units)} courses at day {day} slot {slot} "
                    f"(cross-department)",
                    codes,
                ))
    return violations
